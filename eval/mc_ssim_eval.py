...
def main(original_dir, generated_dir, mask_dir, generated_img_extension):
    ...
    for batch in dataloader:
        ...
        ssim_score += ssim(ori_masked, gen_masked, data_range=255, size_average=True)
        
    return ssim_score / len(dataloader)

if __name__ == "__main__":
    original_dir = '/home/qkrwnstj/StableVITON/dataset/zalando-hd-resized/test/image'
    mask_dir = '/home/qkrwnstj/StableVITON/dataset/zalando-hd-resized/test/agnostic-mask'  # <-- 마스크 경로
    ...
    for name, gen_dir in generated_dir_dict.items():
        SSIM_score = main(original_dir, gen_dir, mask_dir, generated_img_extension)
        print(f"[{name}] Masked SSIM score: {SSIM_score:.4f}")
        total_score += SSIM_score
    
    print(f"[Total] Masked SSIM score: {total_score / 2:.4f}")
